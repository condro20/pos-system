...
// Fungsi ambil data (Search & Sort)
const fetchData = () => {
    // Sesuaikan route name ini jika nama route Anda berbeda (misal: 'pos.history')
...
// Jeda pencarian otomatis saat mengetik
let searchTimeout;
watch(search, () => {
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Riwayat Transaksi Kasir</h2>
                <Link :href="route('pos.index')" class="bg-gray-600 text-white px-4 py-2 rounded-md hover:bg-gray-700 font-bold transition">
        ...
        <div class="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                ...
                <div class="relative w-full md:w-1/3 mb-4">
                    <input v-model="search" type="text" placeholder="Cari No. Invoice atau Nama Kasir..." class="w-full rounded-md border-gray-300 shadow-sm pr-10 focus:border-indigo-500 focus:ring-indigo-500" />
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-100 border-b-2 text-gray-700">
                                ...
                                <th class="p-3 text-right cursor-pointer hover:bg-gray-200 transition select-none" @click="sortBy('grand_total')">
                                    Total Belanja <span v-if="sortField === 'grand_total'" class="text-indigo-600">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span>
                                </th>
                        ...
                        <tbody>
                            <tr v-for="sale in sales.data" :key="sale.id" class="border-b hover:bg-gray-50 transition">
                                ...
                                <td class="p-3">{{ sale.user?.name || 'Kasir Dihapus' }}</td>
                                <td class="p-3">
                                ...
                                <td class="p-3 text-right font-bold text-indigo-600">{{ formatRp(sale.grand_total) }}</td>
                            ...
                            <tr v-if="sales.data.length === 0">
                                <td colspan="6" class="p-6 text-center text-gray-500 font-medium">Belum ada riwayat transaksi.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-3xl overflow-hidden">
                    ...
                    <tbody>
                        <template v-if="selectedData?.sale_details && selectedData.sale_details.length > 0">
                            <tr v-for="detail in selectedData.sale_details" :key="detail.id" class="border-b">
                                <td class="p-2">{{ detail.product?.name || 'Produk Tidak Ditemukan' }}</td>
                        ...
                        <template v-else>
                            <tr>
                                <td colspan="4" class="p-4 text-center text-red-500 font-bold italic">
                                    Data detail tidak ditemukan dari database!
                                </td>
                            </tr>
                        </template>
                    </tbody>
            ...
            <div class="px-6 py-4 border-t bg-gray-50 text-right">
                <button @click="showModal = false" class="bg-gray-800 hover:bg-gray-900 text-white px-6 py-2 rounded shadow font-bold">Tutup</button>
            </div>
        </div>
    </div>
</template>